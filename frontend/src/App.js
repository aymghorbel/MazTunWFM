...
// Palette: Mazarine Blue #0F27A4 + Mazarine Red #F32837 + translucent chrome
const THEMES = {
  enterprise: { key:"enterprise", label:"Mazarine", ico:"🌊", vars:{
    /* Page + surface — neutral page bg gives glass something to refract */
    "--bg":"#F2F3F9","--surface":"#FFFFFF","--s2":"#F4F5F9","--s3":"#E5E7EE",
    "--b":"#E5E7EE","--b2":"#B6BACB",
    /* Ink ramp from the Mazarine brand guide */
    "--t":"#0B1020","--t2":"#32364A","--t3":"#6B7087",
    /* Primary = Mazarine Blue; vd = deeper wordmark blue; vl = soft tint */
    "--v":"#0F27A4","--vl":"#E7EAFB","--vd":"#02178A",
    "--sk":"#0F27A4","--skl":"#E7EAFB",
    /* Status: keep existing semantics but warm them toward brand */
    "--gr":"#107C10","--grl":"#E8F5E9",
    "--am":"#F59E0B","--aml":"#FFF3D1",
    "--re":"#F32837","--rel":"#FDE9EB",
    /* Concentric Liquid Glass radii — 10px cards, 6px inputs/buttons */
    "--r":"10px","--rs":"6px",
    /* Soft specular shadows replace Fluent's hard drops */
    "--sh":"0 1px 0 rgba(255,255,255,.9) inset, 0 4px 14px rgba(11,16,32,.06), 0 1px 2px rgba(11,16,32,.04)",
    "--shm":"0 1px 0 rgba(255,255,255,.95) inset, 0 18px 40px rgba(11,16,32,.12), 0 4px 10px rgba(11,16,32,.06)",
    "--shl":"0 1px 0 rgba(255,255,255,.95) inset, 0 30px 60px rgba(11,16,32,.18), 0 8px 20px rgba(11,16,32,.08)",
    /* Enterprise-scoped tokens kept alive so existing selectors keep working */
    "--ent-sb":"#F4F5F9","--ent-sb-hover":"#EAECF4","--ent-sb-active-bg":"#E7EAFB",
    "--ent-accent":"#0F27A4","--ent-accent-hover":"#02178A",
    "--ent-neutral":"#6B7087","--ent-neutral-light":"#F4F5F9",
    "--ent-on":"#0F27A4","--ent-on-bg":"#E7EAFB",
    "--ent-extra":"#5C2D91","--ent-extra-bg":"#EDE9FE",
    "--ent-hol":"#6B7087","--ent-hol-bg":"#F4F5F9",
    /* Liquid Glass — materials layer tokens */
    "--glass-surface":"rgba(255,255,255,0.62)",
    "--glass-surface-thin":"rgba(255,255,255,0.42)",
    "--glass-surface-thick":"rgba(255,255,255,0.82)",
    "--glass-stroke":"rgba(11,16,32,0.08)",
    "--glass-stroke-strong":"rgba(11,16,32,0.14)",
    "--glass-inner-hi":"rgba(255,255,255,0.85)",
    "--glass-inner-lo":"rgba(255,255,255,0.10)",
    "--glass-tint-blue":"rgba(15,39,164,0.08)",
...
:root{
  --bg:#F2F3F9;--surface:#fff;--s2:#F4F5F9;--s3:#E5E7EE;
  --b:#E5E7EE;--b2:#B6BACB;
  --t:#0B1020;--t2:#32364A;--t3:#6B7087;
  --v:#0F27A4;--vl:#E7EAFB;--vd:#02178A;
  --sk:#0F27A4;--skl:#E7EAFB;
...
.logo-ico{width:36px;height:36px;border-radius:6px;background:linear-gradient(135deg,#0F27A4,#02178A);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;color:#fff;font-family:'JetBrains Mono',monospace;box-shadow:0 1px 0 rgba(255,255,255,.25) inset, 0 6px 14px rgba(15,39,164,.30);}
...
.bulk-btn{padding:6px 13px;border-radius:var(--rs);font-size:12px;font-weight:600;cursor:pointer;border:none;font-family:"Aptos","Segoe UI",-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif;transition:all .12s;}
...
:root[style*="--ent-sb"] *{font-family:"Aptos","Segoe UI","Aptos Display",-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif;}
...
:root[style*="--ent-sb"] .sb{background:rgba(255,255,255,0.72);backdrop-filter:blur(24px) saturate(180%);-webkit-backdrop-filter:blur(24px) saturate(180%);border-right:1px solid var(--b);}
...
/* Enterprise: topbar */
:root[style*="--ent-sb"] .topbar{height:52px;padding:0 24px;border-bottom:none;background:linear-gradient(180deg,#0F27A4 0%,#02178A 100%);box-shadow:0 1px 0 rgba(255,255,255,.15) inset, 0 4px 14px rgba(15,39,164,.28);}
...
:root[style*="--ent-sb"] .card{border-radius:10px;padding:16px 20px;background:rgba(255,255,255,0.82);backdrop-filter:blur(20px) saturate(180%);-webkit-backdrop-filter:blur(20px) saturate(180%);border:1px solid rgba(11,16,32,0.08);box-shadow:var(--sh);transition:box-shadow .22s cubic-bezier(0.2,0.7,0.2,1), transform .22s cubic-bezier(0.2,0.7,0.2,1);}
...
:root[style*="--ent-sb"] .bp{background:var(--v);box-shadow:0 1px 0 rgba(255,255,255,.25) inset, 0 6px 14px rgba(15,39,164,.30);}
:root[style*="--ent-sb"] .bp:hover{background:var(--vd);box-shadow:0 1px 0 rgba(255,255,255,.25) inset, 0 10px 22px rgba(15,39,164,.35);}
...
function Settings({user,users,setUsers,projects,setProjects,roles,setRoles,activities,setActivities,holidays,setHolidays,entities,setEntities,workflows,setWorkflows,onResetPwd,departments=[],setDepartments,balanceTypes=[],setBalanceTypes}) {
  ...
  async function delUser(id){
    if(window.confirm("Remove user?")){
      try{await usersAPI.delete(id);setUsers(p=>p.filter(u=>u.id!==id));setSelectedUserIds(s=>{const n=new Set(s);n.delete(id);return n;});}
      catch(err){toast('Failed to delete user: '+err.message);}
    }
  }
  const [selectedUserIds,setSelectedUserIds]=useState(new Set());
  const deletableUsers=users.filter(u=>u.role!=="superadmin"&&u.id!==user.id);
  const allSelected=deletableUsers.length>0&&deletableUsers.every(u=>selectedUserIds.has(u.id));
  function toggleUserSel(id){setSelectedUserIds(s=>{const n=new Set(s);n.has(id)?n.delete(id):n.add(id);return n;});}
  function toggleAllUsers(){setSelectedUserIds(allSelected?new Set():new Set(deletableUsers.map(u=>u.id)));}
  async function bulkDeleteUsers(){
    if(selectedUserIds.size===0)return;
    if(!window.confirm(`Delete ${selectedUserIds.size} user(s)? This cannot be undone.`))return;
    const ids=[...selectedUserIds];
    const errors=[];
    for(const id of ids){
      try{await usersAPI.delete(id);}
      catch(err){errors.push({id,err:err.message});}
    }
    setUsers(p=>p.filter(u=>!ids.includes(u.id)||errors.some(e=>e.id===u.id)));
    setSelectedUserIds(new Set());
    if(errors.length>0) toast(`Deleted ${ids.length-errors.length}/${ids.length}. ${errors.length} failed.`);
    else toast(`Deleted ${ids.length} user(s).`,"info");
  }
      ...
      {tab==="users"&&(<div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            ...
            <div style={{fontSize:12,color:"var(--t3)"}}>{users.filter(u=>u.active).length} active{selectedUserIds.size>0?` · ${selectedUserIds.size} selected`:""}</div>
          </div>
          <div style={{display:"flex",gap:8}}>
            {selectedUserIds.size>0&&<button className="btn bd bsm" onClick={bulkDeleteUsers}>🗑 Delete {selectedUserIds.size} selected</button>}
            {selectedUserIds.size>0&&<button className="btn bo bsm" onClick={()=>setSelectedUserIds(new Set())}>✕ Clear</button>}
            <button className="btn bg2 bsm" onClick={()=>downloadTemplate('users')}>⬇ Template</button>
            <label className="btn bg2 bsm" style={{cursor:"pointer",margin:0}}><input type="file" accept=".csv,.xlsx" style={{display:"none"}} onChange={e=>{handleImportFile(e,'users');e.target.value='';}} />📥 Import CSV</label>
            <button className="btn bp bsm" onClick={()=>setShowAdd(true)}>+ Add User</button>
          </div>
        </div>
        <div className="tw"><table className="tbl"><thead><tr>
          <th style={{width:36,textAlign:"center"}}><input type="checkbox" className="cb" checked={allSelected} onChange={toggleAllUsers} title="Select all deletable users"/></th>
          <th>User</th><th>Role</th><th>Type</th><th>Dept</th><th>Manager</th><th>Balances</th><th>Active</th><th>2FA</th><th>Actions</th>
        </tr></thead><tbody>{users.map(u=>{
          const canSel=u.role!=="superadmin"&&u.id!==user.id;
          const isSel=selectedUserIds.has(u.id);
          return(
          <tr key={u.id} style={isSel?{background:"var(--vl)"}:undefined}>
            <td style={{textAlign:"center"}}>{canSel?<input type="checkbox" className="cb" checked={isSel} onChange={()=>toggleUserSel(u.id)}/>:<span style={{color:"var(--t3)",fontSize:10}}>—</span>}</td>
            <td><div style={{display:"flex",alignItems:"center",gap:9}}><div className="av" style={{background:aColor(u.id)}}>{initials(u.name)}</div><div><div style={{fontWeight:700,fontSize:13}}>{u.name}</div><div style={{fontSize:11,color:"var(--t3)",fontFamily:"'JetBrains Mono',monospace"}}>{u.email}</div></div></div></td>
            <td><RoleBadge role={u.role} roles={roles}/></td>
            <td><TypeBadge type={u.type}/></td>
            <td style={{fontSize:12,color:"var(--t2)"}}>{u.dept}</td>
            <td style={{fontSize:12,color:"var(--t3)"}}>{users.find(x=>x.id===u.manager)?.name||"—"}</td>
            <td><div style={{display:"flex",flexDirection:"column",gap:3}}><div style={{display:"flex",alignItems:"center",gap:6}}><div className="prog" style={{width:48}}><div className="prog-f" style={{width:`${Math.round((u.usedLeave/u.leaveBalance)*100)}%`,background:u.usedLeave/u.leaveBalance>0.8?"var(--re)":"var(--gr)"}}/></div><span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:"var(--t3)"}} title="Annual Leave remaining">{u.leaveBalance-u.usedLeave}d AL</span></div>{(u.recoveryBalance||0)>0&&<span style={{fontSize:10,color:"var(--v)",fontFamily:"'JetBrains Mono',monospace"}}>🔄 {u.recoveryBalance}d</span>}</div></td>
            <td><label className="sw"><input type="checkbox" checked={u.active} onChange={()=>toggleU(u.id)}/><span className="sldr"/></label></td>
            <td>{u.totpEnabled?<span className="badge bgr" style={{fontSize:10}}>🔒 On</span>:<span className="badge bgr2" style={{fontSize:10}}>Off</span>}</td>
            <td><div style={{display:"flex",gap:4}}><button className="btn bg2 bxs" onClick={()=>setEditUser({...u})}>✏️</button>{onResetPwd&&<button className="btn bg2 bxs" title="Reset password" onClick={()=>onResetPwd(u.id)}>🔑</button>}{u.totpEnabled&&<button className="btn bg2 bxs" title="Reset 2FA" onClick={()=>resetMFA(u.id)}>🔓</button>}{canSel&&<button className="btn bg2 bxs" onClick={()=>delUser(u.id)}>🗑</button>}</div></td>
          </tr>);
        })}</tbody></table></div>
      </div>)}
