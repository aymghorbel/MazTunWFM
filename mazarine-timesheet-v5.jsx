...
import { authAPI, usersAPI, projectsAPI, requestsAPI, timesheetAPI, rolesAPI, payrollAPI, activitiesAPI, rotationAPI, companyAPI, totpAPI, auditAPI, emailAPI, pushAPI, holidaysAPI } from "./api";
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12}}>
        {Array.from({length:12},(_,m)=>{
          ...
          const hrs=ents?ents.reduce((s,e)=>s+e.hours,0):"—";
          ...
          return (
            <div key={m} className="card" style={{cursor:isFuture?"default":"pointer",opacity:isFuture?.55:1,border:`1.5px solid ${borderCol}`,transition:"box-shadow .15s"}}
              ...
              <div style={{display:"flex",gap:12,fontSize:12,color:"var(--t3)"}}>
                ...
                <span>⏱ {hrs}{typeof hrs==="number"?"h":""}</span>
          ...
          <div className="tw">
            <table className="tbl">
              <thead><tr>
                ...
                <th>Activity</th><th>Project Split</th><th>Hours</th><th>Allocated</th><th>Status</th>
              </tr></thead>
              <tbody>
                {entries.map(e=>{
                  ...
                  return (
                    <>
                      <tr key={e.id} className={isSel?"sel-row":""} style={{cursor:canExpand?"pointer":"default",opacity:isLocked?.85:1}}
                        ...
                        <td style={{fontFamily:"'JetBrains Mono',monospace",fontWeight:700}}>{e.hours}h</td>
                      ...
                      {isOpen&&canExpand&&(
                        <tr key={e.id+"_al"}>
                          <td colSpan={user.type==="field"?10:9} style={{padding:0}}>
      ...
      {tab==="projects"&&(
        <div>
          <div className="sg">
            ...
            <div className="sc"><div className="sa" style={{background:"var(--sk)"}}/><div className="sl">Total Hours</div><div className="sv" style={{color:"var(--sk)"}}>{totHours}h</div></div>
          ...
          <div className="g2">
            <div className="tw">
              ...
              <table className="tbl">
                <thead><tr><th>Project</th><th>Type</th><th>Days</th><th>Hours</th><th>Share</th></tr></thead>
                <tbody>
                  {projSummary.map(({pid,proj,days,hours})=>{const pct=totAlloc>0?Math.round((days/totAlloc)*100):0;return(
                    <tr key={pid}><td><div style={{display:"flex",alignItems:"center",gap:8}}><div className="dot" style={{background:proj?.color||"#94a3b8"}}/><div><div style={{fontWeight:700,fontSize:13}}>{proj?.code||"?"}</div><div style={{fontSize:11,color:"var(--t3)"}}>{proj?.name}</div></div></div></td><td><span className="badge bgr2" style={{fontSize:10}}>{proj?.type}</span></td><td style={{fontFamily:"'JetBrains Mono',monospace",fontWeight:700,color:"var(--v)"}}>{days}</td><td style={{fontFamily:"'JetBrains Mono',monospace",color:"var(--t2)"}}>{hours}h</td><td><div style={{display:"flex",alignItems:"center",gap:8}}><div className="prog" style={{width:56}}><div className="prog-f" style={{width:`${pct}%`,background:proj?.color||"var(--v)"}}/></div><span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:"var(--t3)"}}>{pct}%</span></div></td>
                  </tr>);})}
      ...
      {tab==="timesheets"&&(
        <div>
          ...
          {submittedTS.length===0?(
            ...
            <div className="tw">
              <table className="tbl">
                <thead><tr><th>Employee</th><th>Period</th><th>Working Days</th><th>Hours</th><th>Projects</th><th>Submitted</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                      ...
                      <tr key={item.key}>
                        <td>
                          <div style={{display:"flex",alignItems:"center",gap:9}}>
                            ...
                            <div><div style={{fontWeight:700,fontSize:13}}>{item.user.name}</div><div style={{fontSize:11,color:"var(--t3)"}}>{item.user.dept}</div></div>
                          </div>
                        </td>
                        ...
                        <td style={{fontFamily:"'JetBrains Mono',monospace"}}>{hours}h</td>
                        <td>
          ...
          {allTeamReq.map(r=>(
            <div className="rc" key={r.id}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                {r.status==="Pending"&&<><button className="btn bs bxs" onClick={()=>approve(r.id)}>✓</button><button className="btn bd bxs" onClick={()=>reject(r.id)}>✗</button></>}
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="md" style={{maxWidth:820,width:"95vw",maxHeight:"88vh",display:"flex",flexDirection:"column"}}>
              ...
              <div style={{overflow:"auto",flex:1,marginBottom:14}}>
                <table className="tbl" style={{minWidth:560}}>
                  <thead><tr><th>Date</th><th>Day</th><th>Activity</th><th>Hours</th><th>Project Allocation</th></tr></thead>
                  <tbody>
                    {entries.map(e=>(
                      <tr key={e.id}>
                        <td style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:"var(--t3)"}}>{e.date}</td>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {ps.map(({proj,days,hours})=>(
                    <div className="ap-alloc-row" key={proj?.id} style={{background:"var(--s2)",borderRadius:"var(--rs)"}}>
                      ...
                      <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:"var(--t3)",width:40,textAlign:"right"}}>{hours}h</span>
                    </div>
                  ))}
...
function Analytics({user,requests,users,projects,roles,tsStatuses}) {
  const isAd=hasPerm(roles,user.role,"all");
  const teamIds=isAd?users.map(u=>u.id):users.filter(u=>u.manager===user.id).map(u=>u.id);
  const lvData=useMemo(()=>{const m={};requests.filter(r=>teamIds.includes(r.userId)).forEach(r=>{m[r.type]=(m[r.type]||0)+r.daysCount;});return Object.entries(m).map(([n,v],i)=>({name:n,value:v,color:COLORS[i%COLORS.length]}));},[requests,teamIds]);
  const pend=requests.filter(r=>r.status==="Pending").length;
  const alTot=requests.filter(r=>r.status==="Approved"&&r.type==="Annual Leave").reduce((s,r)=>s+r.daysCount,0);
  ...
  return (
    <div>
      ...
      <div className="g2">
        <div className="card"><div className="card-hd"><div><div className="card-title">Monthly Trends</div></div></div>
          <ResponsiveContainer width="100%" height={195}><AreaChart data={MONTHLY_STATS} margin={{top:0,right:0,bottom:0,left:-22}}>
            ...
            <Tooltip content={<CT/>}/><Area type="monotone" dataKey="workedDays" name="Worked Days" stroke="#7c3aed" fill="url(#gw)" strokeWidth={2} dot={false}/><Area type="monotone" dataKey="leaves" name="Leave Days" stroke="#10b981" fill="url(#gl)" strokeWidth={2} dot={false}/>
          </AreaChart></ResponsiveContainer>
        </div>
        <div className="card"><div className="card-hd"><div><div className="card-title">Leave Distribution</div></div></div>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            <div style={{flex:1}}>{lvData.map((e,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><div className="dot" style={{background:e.color}}/><span style={{fontSize:12,color:"var(--t2)",flex:1}}>{e.name}</span><strong style={{fontSize:12,fontFamily:"'JetBrains Mono',monospace"}}>{e.value}d</strong></div>)}</div>
          </div>
        </div>
      </div>
      <div className="g2">
        <div className="card"><div className="card-hd"><div><div className="card-title">Overtime & Missions</div></div></div>
          <ResponsiveContainer width="100%" height={160}><BarChart data={MONTHLY_STATS} margin={{top:0,right:0,bottom:0,left:-22}}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false}/><XAxis dataKey="month" tick={{fontSize:11,fill:"#94a3b8"}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:11,fill:"#94a3b8"}} axisLine={false} tickLine={false}/><Tooltip content={<CT/>}/><Bar dataKey="overtime" name="Overtime" fill="#ef4444" radius={[4,4,0,0]} maxBarSize={26}/><Bar dataKey="missions" name="Missions" fill="#7c3aed" radius={[4,4,0,0]} maxBarSize={26}/></BarChart></ResponsiveContainer>
        </div>
        <div className="card">
          <div className="card-hd"><div className="card-title">Project Activity</div></div>
          {projects.slice(0,5).map((p,i)=>{const pct=40-i*6;return(<div key={p.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:9}}><div className="dot" style={{background:p.color}}/><span style={{fontSize:12,color:"var(--t2)",width:100,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.code}</span><div className="prog" style={{flex:1}}><div className="prog-f" style={{width:`${pct}%`,background:p.color}}/></div><span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:"var(--t3)",width:30,textAlign:"right"}}>{pct}%</span></div>);})}
          {isAd&&<><div className="divider"/><div style={{fontWeight:700,fontSize:13,marginBottom:8}}>Submission Rate</div><ResponsiveContainer width="100%" height={95}><LineChart data={MONTHLY_STATS} margin={{top:0,right:0,bottom:0,left:-22}}><XAxis dataKey="month" tick={{fontSize:10,fill:"#94a3b8"}} axisLine={false} tickLine={false}/><YAxis domain={[80,100]} tick={{fontSize:10,fill:"#94a3b8"}} axisLine={false} tickLine={false} unit="%"/><Tooltip/><Line type="monotone" dataKey="ontime" name="On-time %" stroke="#0ea5e9" strokeWidth={2} dot={{r:3,fill:"#0ea5e9",stroke:"#fff",strokeWidth:2}}/></LineChart></ResponsiveContainer></>}
        </div>
      </div>
    </div>
  );
...
function Settings({user,users,setUsers,projects,setProjects,roles,setRoles,activities,setActivities,holidays,setHolidays,onResetPwd}) {
  ...
  const PTYPES=["OPEX","CAPEX","OVERHEAD","INTERNAL"];
  ...
  return (
    <div>
      <div className="tabs">{[["users","👥 Users & Roles"],["projects","📁 Projects"],["activities","🎯 Activities"],["holidays","📅 Holidays"],["rbac","🔐 Permissions & Roles"],["system","⚙️ System"]].map(([k,l])=>(<div key={k} className={`tab ${tab===k?"active":""}`} onClick={()=>setTab(k)}>{l}</div>))}</div>
      {tab==="users"&&(<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div><div style={{fontWeight:700,fontSize:15}}>User Management</div><div style={{fontSize:12,color:"var(--t3)"}}>{users.filter(u=>u.active).length} active</div></div><button className="btn bp bsm" onClick={()=>setShowAdd(true)}>+ Add User</button></div>
      ...
      {tab==="projects"&&(<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div><div style={{fontWeight:700,fontSize:15}}>Project Management</div><div style={{fontSize:12,color:"var(--t3)"}}>{projects.filter(p=>p.open).length} open · {projects.filter(p=>!p.open).length} closed</div></div><div style={{display:"flex",gap:8}}><div className="tabs" style={{margin:0}}>{[["all","All"],["open","Open"],["closed","Closed"]].map(([k,l])=><div key={k} className={`tab ${pFilter===k?"active":""}`} onClick={()=>setPFilter(k)} style={{padding:"5px 11px"}}>{l}</div>)}</div><button className="btn bp bsm" onClick={()=>setShowAddProj(true)}>+ Add Project</button></div></div>
...
export default function App() {
  ...
  const NAV = [
    ...
    {key:"schedule",   label:"Schedule",    ico:"📅", show:!isSA},
          ...
          <div className="content">
            {isSA && view !== "settings" ? (
              ...
              <>
                ...
                {view==="settings"   && isAd   && <Settings user={user} users={users} setUsers={setUsers} projects={projects} setProjects={setProjects} roles={roles} setRoles={setRoles} onResetPwd={adminResetPwd} activities={activities} setActivities={setActivities} holidays={holidays} setHolidays={setHolidays}/>}
              </>
            )}
          </div>
        </main>
